...
def generate_documents_with_guaranteed_images():
    docx_dest = r"c:\home\practice_2d\luxu-real-estate\luxu-real-estate\Documentacion\Documentacion_Proyecto_Base_de_Datos.docx"
    pdf_dest = r"c:\home\practice_2d\luxu-real-estate\luxu-real-estate\Documentacion\Documentacion_Proyecto_Base_de_Datos.pdf"
    
    root_docx = r"c:\home\practice_2d\luxu-real-estate\luxu-real-estate\Documentacion_Proyecto_Base_de_Datos.docx"
    root_pdf = r"c:\home\practice_2d\luxu-real-estate\luxu-real-estate\Documentacion_Proyecto_Base_de_Datos.pdf"
...
2. Clave-Valor (Redis): Utilizado para la aceleración de caché de propiedades en memoria bajo la clave `all_properties_cache`.""")
    ...
    # 2.7 y 2.8 Creación e Inserción (15 pts.)
    add_h("Creación de las Bases de Datos e Inserción de Registros (15 pts.)", level=2)
    doc.add_paragraph("""- Base de Datos Relacional (PostgreSQL): Creada mediante script DDL con 50 registros reales y coherentes por cada una de las 4 tablas (profiles, properties, user_favorites, appointments), alcanzando 200 registros.
...
- `mongodb_nosql_dataset.json`: Dataset BSON de 10,000 registros para MongoDB.""")
    ...
    # EMBED IMAGES WITH ABSOLUTE GUARANTEE
    images_to_add = [
        ("Figura 1. Pantalla Principal (Landing Page) con Buscador Multicriterio y Filtros por Categorías.", img_home),
        ("Figura 2. Catálogo General de Propiedades (/properties) mostrando los contadores dinámicos y muestras del Dataset NoSQL.", img_cat),
        ("Figura 3. Módulo de Propiedades Favoritas (/favorites) con sincronización e inmunización acumulativa.", img_fav),
        ("Figura 4. Panel Administrativo de Diagnóstico BD (/admin/database) mostrando objetos de base de datos y diccionario.", img_db)
    ]

    for title, path in images_to_add:
    ...
    doc.save(docx_dest)
    doc.save(root_docx)
    print("DOCX files generated with guaranteed images!")
    ...
    for title, path in images_to_add:
        if os.path.exists(path):
            ...
            story.append(RLImage(path, width=6.2*inch, height=3.88*inch))
    ...
    # Copy PDF to root
    with open(pdf_dest, 'rb') as rf, open(root_pdf, 'wb') as wf:
        wf.write(rf.read())

    print("PDF files generated with guaranteed images!")

generate_documents_with_guaranteed_images()
