...
const UpcomingVisitsList: React.FC<UpcomingVisitsListProps> = ({ visits, isLoading }) => {
  ...
  if (visits.length === 0) {
    return <div className="text-center py-12 text-slate-500">No upcoming visits yet.</div>;
  }
          ...
          <CardContent className="flex flex-col sm:flex-row items-start sm:items-center gap-4 p-0">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2 text-sm text-slate-500 mt-1">
                ...
                <span>Agent: {visit.agent_name}</span>
              </div>
            </div>
            <div className="flex w-full sm:w-auto gap-2 mt-2 sm:mt-0">
              <Button variant="outline" className="rounded-full flex-1 sm:flex-none">Reschedule</Button>
              <Button className="rounded-full bg-[#006655] hover:bg-[#005544] flex-1 sm:flex-none">Get Directions</Button>
            </div>
          </CardContent>
        </Card>
      ))}
