...
export default function Home() {
  ...
  const filteredMarketProperties = marketProperties.filter((prop) => {
    ...
    if (filters.type && prop.status !== filters.type.toUpperCase()) return false;
    return true;
  });
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20 space-y-20">
        ...
        <section>
          <div className="flex items-end justify-between mb-8">
            ...
            <Button variant="link" className="text-[#006655] hover:opacity-70 flex items-center gap-1 p-0 h-auto font-medium">
        ...
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredMarketProperties.map((prop) => (
              <PropertyCard key={prop.id} {...prop} />
            ))}
          </div>
          <div className="mt-12 text-center">
            <Button variant="outline" className="px-8 py-6 bg-white border-gray-100 hover:border-[#006655] hover:text-[#006655] text-[#19322F] font-medium rounded-lg transition-all hover:shadow-md">
