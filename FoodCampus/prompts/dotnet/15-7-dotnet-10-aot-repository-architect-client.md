<system_instruction>
  ...
  <technical_landscape>
    El proyecto, [UtmMarket.csproj], es una aplicación de consola en .NET 10 optimizada para el despliegue Native AOT. 
    Restricciones clave:
...
<context>
  <project_metadata>
    Nombre del Proyecto: [UtmMarket.csproj]
  ...
  <reference_files>
    ...
    - src/Infrastructure/Mappers/ClienteMapper.cs
...
<task_description>
  <objective>
    Crear la implementación concreta del Repositorio de Clientes compatible con Native AOT.
  </objective>
  
  <steps>
    <step id="1">
      Crear 'ClientRepositoryImpl.cs' en 'src/Infrastructure/Repositories/'. 
    ...
    <step id="2">
      ...
      - Usar SqlCommand y SqlDataReader para el mapeo manual de ClienteEntity.
      - Evitar llamadas genéricas de Dapper que dependan de reflexión interna.
    </step>
    <step id="3">
      Mapeo: Utilizar 'ClienteMapper' para convertir entidades de persistencia a modelos de dominio.
    </step>
    <step id="4">
      Registro: Registrar 'IClientRepository' con 'ClientRepositoryImpl' en el contenedor de DI.
    </step>
  </steps>
</task_description>
...
<output_requirements>
  <deliverables>
    1. Código completo para 'ClientRepositoryImpl.cs' con mapeo manual.
