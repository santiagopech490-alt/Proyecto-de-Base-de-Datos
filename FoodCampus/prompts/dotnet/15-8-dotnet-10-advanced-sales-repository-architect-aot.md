...
<task_description>
  Desarrollar la implementación concreta del Repositorio de Clientes (ClientRepositoryImpl.cs) optimizada para Native AOT. La implementación debe evitar el problema de consultas N+1 mediante el uso de comandos SQL optimizados y mapeo manual vía SqlDataReader.
</task_description>
...
<execution_steps>
  1. Implementar: Crear ClientRepositoryImpl.cs en 'src/Infrastructure/Repositories/'.
  2. Optimizar: Escribir lógica manual para mapear ClienteEntity desde un DataReader.
  3. Mapeo: Utilizar ClienteMapper.cs para la conversión a objetos de dominio.
  4. Registro: Actualizar Program.cs asegurando que el repositorio se resuelva dentro de un manual 'IServiceScope'.
</execution_steps>

<output_format>
  - Código fuente completo para ClientRepositoryImpl.cs con mapeo manual.
