...
<context>
  ...
  Objetivo actual: Definir los contratos (interfaces) de los Casos de Uso para el dominio de Clientes.
  
  Archivos de referencia en el espacio de trabajo:
  - Dominio: Cliente.cs (Modelo de dominio)
  - Infraestructura: ClienteEntity.cs (Entidad de BD), ClientRepositoryImpl.cs (Implementación)
  - Mapeo: ClienteMapper.cs
  - Persistencia: IClientRepository.cs (Contrato del repositorio)
</context>

<task_description>
  Debes generar exclusivamente los contratos (interfaces) para los Casos de Uso de gestión de clientes. Cada acción debe estar aislada en su propio contrato siguiendo el Principio de Segregación de Interfaces (ISP) y Responsabilidad Única (SRP).
  
  Acciones a implementar:
  1. Retrieve all clients (Obtener todos)
  2. Retrieve a client by ID (Obtener por ID)
  3. Retrieve clients using filters (Filtrado dinámico vía ClientFilter)
  4. Create a new client (Crear)
  5. Update an existing client (Actualizar)
  6. Change client activation status (Activar/Desactivar cliente - Caso de uso específico)
</task_description>

<technical_constraints>
  <rule_set name="Arquitectura y Diseño">
    - Capa Destino: Core / Application Layer.
    - Dependencias: Los casos de uso deben interactuar únicamente con objetos de Dominio (Client.cs), nunca con Entidades (ClienteEntity.cs).
  ...
  <rule_set name="Nomenclatura y Ubicación">
    ...
    - Convención de nombres: I[ActionName]UseCase.cs (Ejemplo: ICreateClientUseCase.cs).
    - Idioma: Todo el código, nombres de clases y miembros deben estar estrictamente en Inglés.
  </rule_set>
</technical_constraints>

<execution_steps>
  1. Analizar los archivos existentes para asegurar la compatibilidad de tipos con el modelo 'Client'.
  2. Identificar si la acción de filtrado requiere un objeto DTO de entrada (ClientFilter) y definir su estructura si no existe.
  ...
  5. Proporcionar un resumen de las interfaces creadas y sugerencias si detectas inconsistencias en el `IClientRepository` actual.
</execution_steps>
